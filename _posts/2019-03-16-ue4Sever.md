...
一个replicated actor会同时在服务器和客服端创建，但分配不同的角色（role）。一般服务器actor的角色为`ROLE_Authority`，控制了actor的状态，并向客服端的actor同步状态信息。客服端为`ROLE_SimulatedProxy`或者`ROLE_AutonomousProxy`。

remote role是actor在对应远端机器上对应的角色。比如在服务器上Role == ROLE_Authority，RemoteRole == ROLE_SimulatedProxy。
...
组件RPC会有额外的消耗。移动组件通过组件的ower代理RPC来减少RPC的消耗。
...
一个角色的移动在主控端（autonomous proxy）和其它端（simulated proxies）的同步方式是不一样的。其他端只需要保持更新角色位置就行了，而主控端必须在位置修正和玩家平滑体验找到最佳平衡点。
...
ue4自带的同步框架，后端共用前端代码，能够快速的开发出多人游戏，但是前后端的代码混在一起缺不利于前后端的分工和后端的优化。除了状态同步之外，E4引擎没有现成的数据库和缓存架构，服务器架构也单一，没有服务器间的调用。
